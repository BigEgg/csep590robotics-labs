...
class GUIWindow():
    ...
    def drawMarkers(self):
        for marker in self.markers:
            ...
            self.colorLine((marker_x, marker_y),
    ...
    def _show_robot(self, robot):
        ...
        self.colorLine(coord,
                       (coord[0]+fov_lx, coord[1]+fov_ly),
                       color='#222222',
                       linewidth=2,dashed=True)
        self.colorLine(coord,
                       (coord[0]+fov_rx, coord[1]+fov_ry),
                       color='#222222',
                       linewidth=2,
                       dashed=True)
    ...
    def colorLine(self, coord1, coord2, color='black', linewidth=1, dashed=False):
        if dashed:
            self.canvas.create_line(coord1[0] * self.grid.scale,
                                    (self.height - coord1[1]) * self.grid.scale,
                                    coord2[0] * self.grid.scale,
                                    (self.height - coord2[1]) * self.grid.scale,
